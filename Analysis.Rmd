...
dat <- droplevels(dat[!dat$authors == 'Polak et al',])
...
```{r}
dat <- filter(dat, parent %in% c("female","both"))

m5a <- rma.mv(yi = lnRR, V = 1, mod = ~parent-1, random = list(~1|esID,
                                               ~1|paperID),
                 test = "t",
                 data = dat)
summary(m5a) 
...
orchard5 <- orchard_plot(m5a, mod = "parent", xlab = "lnRR", alpha=0.4) + 
  geom_errorbarh(aes(xmin = lowerPR, xmax = upperPR), height = 0, show.legend = FALSE, size = 1.1, alpha = 0.5) + # prediction intervals
  geom_errorbarh(aes(xmin = lowerCL, xmax = upperCL), height = 0.05, show.legend = FALSE, size = 2) + # confidence intervals
  scale_colour_manual(values = c("grey34","grey34")) +
  scale_fill_manual(values=c("grey34","grey34")) +
...
```{r}
m5b <- rma.mv(yi = lnRR, V = 1, mod = ~lifestage-1, random = list(~1|esID,
                                               ~1|paperID),
                 test = "t",
                 data = dat)
summary(m5b) 

orchard6 <- orchard_plot(m5b, mod = "lifestage", xlab = "lnRR", alpha=0.4) + 
  geom_errorbarh(aes(xmin = lowerPR, xmax = upperPR), height = 0, show.legend = FALSE, size = 1.1, alpha = 0.5) + # prediction intervals
  geom_errorbarh(aes(xmin = lowerCL, xmax = upperCL), height = 0.05, show.legend = FALSE, size = 2) + # confidence intervals
  scale_colour_manual(values = c("grey34","grey34")) +
  scale_fill_manual(values=c("grey34","grey34")) +
